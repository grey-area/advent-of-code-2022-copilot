...
class Graph:
    ...
    def __init__(self, nodes_dict, shortest_paths=None, exclude_nodes=set()):
        self.nodes_dict = nodes_dict
        self.start_node = 'AA'
        self.nonzero_nodes = {name for name, node in nodes_dict.items() if node.flow > 0}
        self.nonzero_nodes -= exclude_nodes
        if shortest_paths:
            self.shortest_paths = shortest_paths
        else:
            self.shortest_paths = self.find_shortest_paths(self.nonzero_nodes | {self.start_node})

    def path_generator_helper(self, path, unvisited_nodes, time, score=0):
    ...
    def compute_one_agent_answer(self, time):
        best_score = 0
    ...
    def partition_generator(self):
        for k in range(1, len(self.nonzero_nodes) // 2 + 1):
            for partition in combinations(self.nonzero_nodes, k):
                left = set(partition)
    ...
    def compute_two_agent_answer(self, time):
        best_score = 0
        for left_graph, right_graph in self.partition_generator():
            left_score = left_graph.compute_one_agent_answer(time)
            right_score = right_graph.compute_one_agent_answer(time)
...
if __name__ == "__main__":
    ...
    answer = graph.compute_two_agent_answer(time=26)
    print(answer)